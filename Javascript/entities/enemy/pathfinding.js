import { sampleXRange, sampleYRange } from '../player/collision.js';

export function findPath(level, startPx, endPx, entitySize) {
    if (!level || typeof level.tileSize !== 'number' || typeof level.isSolidAt !== 'function') return null;
    ...
    var isWalkable = function (n) {
        ...
        if (sampleXRange(level, top, left, right, 3)) return false;
        if (sampleXRange(level, bottom, left, right, 3)) return false;
        if (sampleYRange(level, left, top, bottom, 3)) return false;
        if (sampleYRange(level, right, top, bottom, 3)) return false;
        return true;
    };
...
if (typeof window !== 'undefined') {
    ...
    window.Pathfinding.findPath = function (level, startPx, endPx, entitySize) {
        return findPath(level, startPx, endPx, entitySize);
    };
}
